...
from mlp_Detection10samp import HiddenLayer
from mlp_Detection10samp import ReturnValue
...
def evaluate_lenet5(learning_rate=0.01, n_epochs=1,
    ...
    tx = numpy.asarray(a[:,:],dtype=theano.config.floatX); #2819
    ...
    ty1 = numpy.asarray(ab[:,:],dtype=theano.config.floatX);
    train_set_y1 =  theano.shared(ty1,borrow=True);
    ty = numpy.asarray(c[:,:],dtype=theano.config.floatX);
    ...
    vx = numpy.asarray(a[300:330,:],dtype=theano.config.floatX);#2819:3315
    valid_set_x =  theano.shared(vx,borrow=True);
    vy = numpy.asarray(c[300:330,:],dtype=theano.config.floatX);
    ...
    ttx = numpy.asarray(a[300:330,:],dtype=theano.config.floatX); #3316:3851
    test_set_x =  theano.shared(ttx,borrow=True);
    vvy = numpy.asarray(c[300:330,:],dtype=theano.config.floatX);
    ...
    test_modelR = theano.function([index], layerR5.Nerrors(y2),
             givens={
                x: test_set_x[index * batch_size: (index + 1) * batch_size],
                y2: test_set_y2[index * batch_size: (index + 1) * batch_size]})

    validate_modelR = theano.function([index], layerR5.Nerrors(y2),
            givens={
                x: valid_set_x[index * batch_size: (index + 1) * batch_size],
                y2: valid_set_y2[index * batch_size: (index + 1) * batch_size]})
    
    train_modelR = theano.function([index], layerR5.Nerrors(y2),
             givens={
                x: train_set_x[index * batch_size: (index + 1) * batch_size],
   ...
   #          givens={
    ...
    save_file = open('resultDetection_50samp_5_10_15.dat', 'wb')
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                train_losses = [train_modelR(i) for i
                                in xrange(n_train_batches)]
                this_train_loss = numpy.mean(train_losses)
                print('epoch %i, minibatch %i/%i, Train error %f %%' % \
                      (epoch, minibatch_index + 1, n_train_batches, \
                ...
                this_validation_loss = numpy.min(validation_losses)
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    '''if(((this_validation_loss - best_validation_loss) > -1) & (learning_rate==0.001)):
                        learning_rate = 0.0001
                        print(learning_rate)'''
